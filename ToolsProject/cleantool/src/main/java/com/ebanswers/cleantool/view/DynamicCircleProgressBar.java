package com.ebanswers.cleantool.view;
...
/**
 * Created by Callanna on 2016/9/14.
 */
public class DynamicCircleProgressBar  extends View {
    /**
     * 进度条所占用的角度
     */
    ...
    public DynamicCircleProgressBar(Context context) {
        super(context);
        init();
    }


    public DynamicCircleProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }


    public DynamicCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
...
//        textPaint.setTypeface(Typeface.createFromAsset(getContext().getAssets(), "fangz.ttf"));
    ...
    public void setProgress(float progress) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                float oldProgress = DynamicCircleProgressBar.this.progress;
                for (int i = 1; i <= 100; i++) {
                    DynamicCircleProgressBar.this.progress = oldProgress + (validProgress - oldProgress) * (1.0f * i / 100);
                    postInvalidate();
